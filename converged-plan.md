# Converged Plan — SRS Multi-Ingest Cutover (HLS Playback) + Venue Builder Embed

**Status**: TEMPORARY coordination reference. Repo-local plan docs in `cur8-api` and `cur8-ui` are becoming the execution source of truth. `PLAN.md` and this file should be removed after final cleanup review confirms required validation gates, release sequence, branch references, and intentionally kept ledger/effort content are preserved elsewhere or deliberately discarded. Broader Wave A/B/C/D scope remains **paused**.
...
- **2026-05-26 — per-repo plan docs landed**: `cur8-api:feat/srs:SRS-PLAN.md`, `cur8-api:feat/venue-builder-api:VENUE-PLAN.md`, `cur8-ui:feat/srs-ui:SRS-PLAN.md`, `cur8-ui:feat/venue-builder-ui:VENUE-PLAN.md` — all docs-only branches off current `staging` / `dev`. Each branch still diffs to exactly one plan file from its base.
...
2. **HLS-only playback, multi-ingest publishing**. SRS adapts to the existing `{playback_url, playback_mode, playback_status, stream, video}` contract. Going forward, viewers watch via HLS **only** in first-live — no SRS WebRTC viewer path unless product later chooses WHEP for low-latency viewing. Publishing remains multi-ingest: OBS/external tools publish via RTMP or RTMPS, and browser / WHIP-capable tools publish via WebRTC-over-WHIP. The `@antmedia/webrtc_adaptor` WebRTC playback paths in `cur8-ui` (currently in `LivestreamWatchPage` and `LibraryFilmReview`) come out as part of the cutover.
...
### 1.1 HLS-only for viewing; multi-ingest for publishing

Two separate dimensions, easy to conflate:

- **Viewing** is HLS-only in first-live. SRS will not introduce a WebRTC viewer path in PR 1–3; viewer playback continues through the existing `/api/*/streams/:viewer_id/master.m3u8` proxy contract that already serves AMS HLS today. If product later needs sub-second viewer latency, add SRS WHEP as a separate playback workstream.
- **Publishing** must support OBS, browser publishers, and third-party streaming tools/services. Required first-live ingest protocols:
  - **RTMP** for OBS and broad custom-ingest compatibility.
  - **RTMPS** for tools/services that require encrypted RTMP or port-443 style ingest.
  - **WHIP** for browser/WebRTC publish and WHIP-capable encoders. WHIP is still WebRTC; it replaces Ant Media's vendor-specific signaling, not the ability to publish WebRTC streams.
- Today's browser publish is wired through `@antmedia/webrtc_adaptor.publish()` — confirmed in use at `LibraryFilmReview/index.js:232` (reviewer-recording feature). Going forward, browser publish is wired through **SRS WHIP** (WebRTC-HTTP Ingestion Protocol — SRS-native). The `whip-client.js` salvage from `feature/repo-upgrades` (+209 lines) becomes mandatory, not conditional.
...
- WebRTC **publish** in `LibraryFilmReview` (`adapterRef.publish(...)`) → swapped to WHIP via `whip-client.js`, hitting SRS WHIP endpoint instead of AMS WebRTC. WebRTC publishing remains supported; only the Ant Media-specific adaptor/signaling is removed.
...
Every callback uses `validateCallback`. RTMP, RTMPS, and WHIP publishers hit `on-publish` / `validate-publish` — SRS abstracts the ingest protocol on this surface after the connection reaches the per-stream task. If route prefix changes, update `streaming-service/srs.conf` and ECS callback-URL generation in the same commit.
...
### 4.5 Scheduled streams + ingest routing (RTMP/RTMPS + WHIP)
...
- **Router service** (always-on, separate from per-stream SRS tasks):
  ...
  - **Availability posture**: single-instance with ECS auto-replacement on task failure (~30–60s recovery). During that window:
    - No new streams can start (RTMP/RTMPS handshake and WHIP signaling all need the router).
    - **Active RTMP/RTMPS publishes break** — RTMP is a long-lived TCP connection proxied through the router for the stream's full duration. OBS/external publishers will need to reconnect once the router is back. Worth flagging for the validation gate.
  ...
  - **RTMPS**: required for OBS/users/services that need encrypted RTMP ingest. Default first-live shape: NLB TLS termination for `rtmps://srs.<env>.cur8.com/<app>/<stream_id>` (prefer port 443 if it can safely share the listener with WHIP HTTPS; otherwise document and expose an explicit RTMPS port such as 1443), forwarding decrypted RTMP bytes to the same router parser as plain RTMP. If 443 is shared, the router must protocol-sniff HTTP WHIP requests vs RTMP publish handshakes after TLS termination. Merge gate includes RTMPS handshake/publish coverage, certificate/SNI validation, and at least one OBS or external-service RTMPS smoke.
...
- **Per-task UDP addressability** (PR 1 decision §12.6). Each per-stream task must advertise a public UDP candidate the browser can reach. Three real options:
  ...
  - **Security group split**:
    - Router service SG: accepts public TCP `443` (WHIP HTTPS at the NLB listener and RTMPS if the shared-443 shape is chosen), public TCP `1935` (RTMP), and any explicitly chosen RTMPS fallback port (for example `1443`) if shared 443 is rejected in PR 1.
    ...
    - Effect: clients cannot bypass the router for RTMP, RTMPS, or WHIP, browser UDP media reaches the advertised candidate, and cur8-api can reach SRS's HTTP API for readiness and sweep checks without round-tripping through the router. Gate 14 (§4.9) verifies the public-bypass refusal; an additional smoke check should confirm the cur8-api → task `:1985` probe succeeds in dev before §4.5 readiness is considered wired.
...
- **Public ingress transport shape**: default first-live shape is one NLB with TLS termination for HTTPS WHIP signaling and RTMPS, forwarding decrypted HTTP/RTMP bytes to the router, plus a TCP 1935 listener that passes plain RTMP to the same router task. Preferred user-facing endpoints are `https://srs.<env>.cur8.com/<app>/<stream_id>/whip`, `rtmp://srs.<env>.cur8.com/<app>/<stream_id>`, and `rtmps://srs.<env>.cur8.com/<app>/<stream_id>` (port 443 preferred for RTMPS if the router can safely protocol-sniff shared TLS traffic; otherwise expose/document the explicit RTMPS port). If PR 1 chooses router-terminated TLS instead, say so explicitly and update router code/tests/certs in the same PR. UDP for RTC media bypasses the NLB entirely — flows browser → per-task public IP directly. **ALB alone is not sufficient** — it cannot carry RTMP-as-TCP-passthrough cleanly and cannot carry UDP at all (though UDP isn't routed through the NLB here either; the constraint applies if anyone tries to route RTC media through the ingress LB).
...
- **SRS publish credential contract**: publishing surfaces keep `stream_publish_token`, and for `streaming.provider = srs` add `{stream_id, whip_url, rtmp_url, rtmps_url, ingest_ready}`. `rtmp_url` / `rtmps_url` are stable public ingest URLs for OBS and third-party services; the stream key is the existing publish token/stream key contract chosen for this flow. `whip_url` is a fully resolved public HTTPS URL pointing at the router (router routes by stream_id in the path) — generated only when the per-stream Fargate task is RUNNING + healthy + has a public UDP candidate. Otherwise `whip_url` is `null` with `ingest_ready: false` so the UI can show "stream ingest not ready" and retry. WHIP token transport is one explicit choice per config — either `Authorization: Bearer <stream_publish_token>` or token-in-query — and `whip-client.js`, SRS config, and tests must match.
- No prod cutover until OBS RTMP publish, OBS/external-service RTMPS publish, browser WHIP publish, and HLS playback through the existing cur8-api HLS proxy all work end-to-end.
...
- SRT ingest for encoder/vMix-style workflows. SRS supports SRT, but first-live scope is RTMP/RTMPS + WHIP unless a concrete customer blocker requires SRT.
...
- SRS publish credential response: public `rtmp_url`, `rtmps_url`, and `whip_url` generated only when the per-stream SRS task is running, SRS/container health is ready, the router route exists, and the task public UDP candidate is known for WHIP; no private task IP leaked to the browser; token/header/query convention matches `streaming-service/srs.conf`.
- Router contract: RTMP publish-command stream-id extraction, RTMPS TLS/listener/protocol-sniffing behavior for the selected ingress shape, WHIP public-path → SRS-native `/rtc/v1/whip/` translation with exact token transport, WHIP SDP pass-through, Redis miss/stale mapping behavior, long-lived TCP backpressure/timeout handling, and CORS preflight for WHIP.
...
- OBS RTMP publish through intended public endpoint.
- OBS RTMPS publish through intended public endpoint.
- At least one third-party/custom streaming service publishes through RTMP or RTMPS using the documented credentials.
...
2. OBS publishes through public RTMPS endpoint.
3. At least one third-party/custom streaming service publishes through RTMP or RTMPS using the documented credentials.
4. **Browser publishes through public SRS WHIP endpoint** (validated against the same `stream_publish_token` flow `LibraryFilmReview` uses today, with no private task IP in the browser URL; router translates the public WHIP route to SRS's native `/rtc/v1/whip/` endpoint and preserves the agreed token transport).
5. API validates publish token + maps to SRS task for all first-live ingest protocols; callback source identity follows §4.5 and does not rely on a task ARN unless explicitly templated into the callback.
6. SRS produces 1080/720/480 ABR HLS.
7. Segments + playlists reach S3.
8. cur8-api serves/proxies playlists + signed segment URLs.
9. cur8-ui plays live HLS on desktop Safari / Chrome / Firefox **+ iOS Safari** (native HLS path).
10. Scheduled / pre-recorded flow.
11. Multi-concurrent without cross-routing (mix of RTMP, RTMPS, and WHIP publishers).
12. Stream end → archived VOD behavior.
13. AMS-HLS rollback documented + tested before prod flip.
14. **Security-group bypass negative test**: from an untrusted/public host (one **not** in the router SG and **not** in the cur8-api/app SG), direct TCP connections to a per-stream task's `1935` (RTMP) and `1985` (SRS HTTP API) are refused at the SG layer. Verifies the §4.5 SG split prevents clients from skipping the router while preserving the legitimate cur8-api → task `:1985` readiness/sweep path. Positive sub-checks on the same ruleset: (a) cur8-api → task `:1985` succeeds, (b) browser UDP RTC media to the advertised candidate port succeeds, (c) router → task `:1935` and `:1985` succeed.
15. **Launch-timeout cleanup test**: force a task that reaches `RUNNING` only after the ~30s readiness cap. API returns `ingest_ready: false` with no URLs, records `'launch_failed'`, calls `StopTask` when possible, leaves no Redis route, and the sweep stops any late-running stale task.
...
- **Admin/client publish-detail views**: where AMS-style publisher details are rendered, add SRS RTMP URL, RTMPS URL, WHIP URL where relevant, and stream key / Bearer token guidance when the resolved provider is SRS.
...
- **Publish** computes and uploads a content-addressed `.mst` audit object first, then materializes/version-flips `venue_section`/`venue_row`/`venue_seat` runtime rows and promotes the same rendered template to the canonical S3 object key the current renderer fetches through `REACT_APP_VENUE_URL` (`${venue_builder.template_prefix}${location.id}.mst` in `venue_builder.template_bucket`). `REACT_APP_VENUE_URL` is a frontend public URL prefix, not an API-side S3 key.
...
- Publishing is explicitly multi-ingest: RTMP and RTMPS for OBS/external services, WHIP for browser/WebRTC publishers and WHIP-capable encoders. WHIP preserves WebRTC publishing while removing Ant Media-specific signaling.
...
- Publish records draft version for support traceability + promotes the generated `.mst` to the API-side canonical S3 object key that the existing renderer reaches through its public venue URL.
...
1. PR 1 lands. Deploy cur8-api to staging with `streaming.provider` unset → no app behavior change; router service + per-stream task definition + RTMP/RTMPS/WHIP ingress infrastructure exist but no traffic flows yet and no per-stream tasks are running. Idle cost is just the always-on router (~$22/mo at 0.25 vCPU Fargate). Per-stream tasks accrue zero cost until PR 2 starts launching them.
2. PR 2 lands. Deploy to dev. Flip `streaming.provider = srs` in dev. Run §4.9 gates, including public RTMP, RTMPS, HTTPS WHIP ingress, browser media connectivity, and third-party/custom RTMP(S) publish.
...
- `feat/srs` (cur8-api): **8–12 days**. Foundation port mechanical (1–2 d). Router service implementation + Redis state machine (3–5 d, long pole per PLAN.md §W1 #3 estimate of ~1 week). `vendor/hls.js` + `controllers/hls.js` extension + `retrieveAndStoreStreamInfo` provider branch (1–2 d). Public RTMP/RTMPS/WHIP ingress + per-task UDP addressability wiring + SRS publish credential contract + readiness probe + immediate-StopTask cleanup + stale-launch sweep (2–2.5 d, expanded for RTMPS ingress, v19 lifecycle hardening, and external-service smoke). Tests + smoke (1.5–2 d, expanded for v19 resolver/sweep/router-translation/callback-identity/RTMPS gates).
- `feat/srs-ui` (cur8-ui): 3–5 days. Admin-side RTMP/RTMPS/WHIP credential display + WHIP client port/audit + film-review publish swap + WebRTC playback removal + `@antmedia/webrtc_adaptor` removal + browser publish manual validation + iOS verification.
...
2. **ShowTix plan-doc cleanup posture** — `converged-plan.md` and `PLAN.md` are temporary scaffolding. Delete them only after final cleanup review confirms repo-local plans preserve the required execution details or intentionally discard ledger/effort history.
