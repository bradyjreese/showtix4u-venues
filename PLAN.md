...
- **HEAD**: `16313884d` `feat(srs): W1 Phase 0 — neutral streaming_streams table + db layer` (unpushed at time of snapshot — see Execution log)
- **Base**: `staging`
...
- **Commits this program**: 12 on the branch since cutting off `staging` (4 prior + 7 Wave D-mirror dep replacements + 1 W1 Phase 0 prep). **Wave D-mirror queue is COMPLETE; W1 Phase 0 (infra-independent data layer) landed `16313884d`.**
...
- **Workstreams in progress + not yet started**:
  - **W1 SRS port — Phase 0 LANDED (`16313884d`)**: neutral `streaming_streams` table migration + `db/streaming-streams.js` mirroring all 7 `db/ams-streams.js` exports. Infra-independent; no AMS code touched (locked decision #8 + the 2026-05-17 AMS-infra-hands-off guardrail). Next phases: salvage controllers/srs.js + vendor/srs.js + streaming-service/* from feature/streaming-service; CDK infra (`infra/lib/srs-stack.ts` already drafted on that branch); RTMP router (the long pole, ~1 week alone); tests. See §"Workstream 1" for full task list.
...
- **Resume command from a fresh chat**: `cd ~/Code/cur8-api && git pull && pnpm install` — verify lint + node-check clean. W1 Phase 0 (`16313884d`: streaming_streams table + db layer) is already landed. **Next: W1 Phase 1 — salvage the SRS code from `feature/streaming-service`** (port, don't rebase). Concrete file list discovered 2026-05-17 in the salvage survey: NEW SRS-only files `controllers/srs.js` (HTTP callbacks: on_publish, on_unpublish, on_hls, on_play + presigned HLS proxy), `vendor/srs.js` (token gen/validate, generateStreamId, createLiveEvent), `streaming-service/{Dockerfile,docker-compose.yml,entrypoint.sh,srs.conf,README.md}` (the SRS container itself), `infra/{cdk.json,package.json,tsconfig.json,bin/infra.ts,lib/srs-stack.ts}` (CDK stack — ECS+ECR+IAM+VPC plumbing already drafted), `docs/srs-fargate-migration-plan.md` (architecture doc). MODIFIED files to carefully re-port (10): `controllers/{streaming,ant-media,events,library,notifications,communications}.js`, `vendor/{ecs,s3,tus,ant-media}.js`, `tasks/scheduled-streams.js`. **Critical: `ant-media.js` + `terraform/ant-media/` are PRESERVED — locked #8 + 2026-05-17 user guardrail.** The salvage branch's `db/migrations/2026-03/add-task-arn-to-ams-streams.sql` is NOT ported (Phase 0 used a neutral table instead per PLAN §W1 #7). After Phase 1: CDK infra in cur8-dev (user provisions per locked #1 AWS perms), RTMP router (~1 week, the long pole, Node-proxy Fargate task per locked #3), tests (the old branch had zero — full list at W1 §5). All SRS code uses `@aws-sdk/*` v3 from day one per locked #4 (already true on the salvage branch — vendor/srs.js uses `crypto.createHash` not aws-sdk; controllers/srs.js needs the v3 swap during port).
