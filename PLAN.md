...
- **`pnpm lint`**: ✅ green post `1c4ccf0d1` (was FAILS exit 1 with 1 error + 3,866 warnings; PrivateRoute deletion + format sweep cleared it). **195 warnings (0 errors)** as of 2026-05-17 Codex re-audit — my earlier "23" was misreading the "23 fixable" count as the total. 23 of the 195 are auto-fixable via `--fix`; the rest are dead-code / unused-var nits inherited from the pre-upgrade code. Cleanup is optional polish.
...
- **Behind `origin/dev`**: 9 commits as of 2026-05-17 Codex re-audit (was 5; dev moved forward 4 more). Original 5 (`35d69519b` revert, `4d24cb8f3` Konva replicate, `56a1b2cd8` + `458c1208c` + `53d98b182` CUR8-3124 trilogy) plus 4 newer. Integration branch will need a sync before any merge back to `dev` — gap is widening, but no W2 Docker readiness yet so PR isn't viable regardless.
...
- **HEAD**: `9c2101462` `feat(srs): W1 Phase 2a — port SRS ECS helpers into vendor/ecs.js (preserve AMS)` (pushed to `origin/feat/upgrade-2026q2`)
- **Base**: `staging`
- **Build**: lint clean (0 errors, 22 warnings — unchanged through all 7 dep-replacement commits + W1 Phase 0); `node --check server.js` clean; `node -e "require('./server.js')"` loads every dep cleanly (errors only on missing local config — expected per §"Validation environments")
- **`pnpm outdated`**: **0 deprecated direct deps remain.** ✅ aws-sdk v2 dropped 2026-05-17 (`8909e8ba5`); fluent-ffmpeg dropped 2026-05-17 (`7d6bb24fc`); sib-api-v3-sdk dropped 2026-05-17 (`24599a4ac`); moment + moment-timezone dropped 2026-05-17 (`9eec44fa8`). `moment` stays as a transitive of `bunyan@1.8.15` (logger; bunyan→pino is a PLAN-deferred swap, not in current scope). **3 patch-level updates available** as of 2026-05-17 Codex re-audit: `qs` 6.15.1→6.15.2, `oxlint` 1.64.0→1.65.0, `oxfmt` 0.49.0→0.50.0. Sweep before any merge to `staging`.
- **`pnpm audit`**: 3 vulns (0 critical / 1 high / 1 moderate / 1 low) — was 105 at W0. All 3 are mocha transitive (`mocha → serialize-javascript` — 1 high RCE via RegExp.flags + 1 moderate DoS via crafted array-likes; `mocha → diff` — 1 low jsdiff DoS in parsePatch/applyPatch). Wave D-mirror complete drove the deprecated-package backlog to 0 vulns (`aws-sdk` v2 low cleared 2026-05-17 with the v3 migration `8909e8ba5`). **Decision 2026-05-17 (user)**: accept these 3 vulns. mocha is at latest (11.7.5) — no upstream fix available. Per user preference no `pnpm.overrides` (would mask the issue; if mocha bumps these, we should pick it up via `pnpm up` not via override pin). Dev-only blast radius (mocha is a test runner, not in production runtime). Re-evaluate when mocha 12 lands.
- **PR #3630**: open as draft against `staging`, **422 files** as of 2026-05-17 Codex re-audit (was 396; staging diverged further with merged PRs landing in parallel). **Mergeable state at last check: DIRTY (CONFLICTING).** Agent-opened 2026-05-15T22:35:41Z (see §"Execution log" → "2026-05-15 night — branch reorg" for context). Per user direction "do nothing with it right now." **Do not spend review time re-checking PR/GitHub state unless the user explicitly asks.** Current reviews should stay local-only: working tree status, branch divergence, build/lint/format/audit, and source inspection. Revisit PR metadata only when SRS wiring (W1 Phase 2+) and Docker (W2) are ready enough for merge planning.
- **Commits this program**: 19 on the branch since cutting off `staging` (4 prior + 7 Wave D-mirror + 3 W1 prep/port + 4 audit fix-ups + 1 W1 Phase 2a `9c2101462`). **Wave D-mirror COMPLETE; W1 Phase 0 + 1 LANDED + 4 audit-fix-ups + Phase 2a LANDED.**
- **What's done**: W0 (Node 24.15.0, pnpm 11.1.2 via Corepack, golden harness committed) → Wave A linter/formatter (oxlint + oxfmt across 370 files) → `pnpm up --latest` covering Wave A test stack (sinon 5→22, mocha 10→11, chai 4→6, chai-http 4→5, nodemon 2→3) + Wave B (18 minor/patch bumps) + Wave C breakers (helmet 3→8, multer 1→2, axios 0.21→1, connect-redis 3→9, config 1→4, csv-stringify 3→6, deepmerge 2→4, pdfmake 0.1→0.3, google-auth-library 7→10, uuid 9→14, yaml 1→2, stripe 13→22, sitemap 2→9, intuit-oauth 3→4, body-parser 1→2, html-to-text 9→10, redis 4→5) + Express 4→5 → **Wave D-mirror dep replacements (7 of 7 landed — COMPLETE)**: `randomized-string` → `crypto.randomBytes` (`3ec996ffd`), `promise-mysql` + legacy `mysql` (full retirement) → `mysql2` including knex client config swap (`959f06bd1`), `@google/maps` → `@googlemaps/google-maps-services-js` (`457ad59f5`), `moment` + `moment-timezone` → `dayjs` via centralized `lib/dayjs.js` (38 files; 13 plugins; inline fixes for moment-tz `Zone.utcOffset` sign convention + immutable-mutation in 2 test files' `roundSecs`) (`9eec44fa8`), `sib-api-v3-sdk` → `@getbrevo/brevo` v5 namespaced client (1 real call site `vendor/mail.js`; Brevo v5 ≠ drop-in for legacy SibApiV3Sdk classes; payload becomes plain object instead of `new SendSmtpEmail()`) (`24599a4ac`), `fluent-ffmpeg` → native `child_process.spawn` of ffmpeg + ffprobe binaries (1 file `tasks/video-processing.js`; AMS pipeline; ffprobe wrapped as Promise helper; `-progress pipe:1` flag for clean key=value progress parsing on stdout instead of fluent-ffmpeg's stderr-log parsing; durationSeconds captured in getFrameRate's ffprobe so percent calc has it without a third call) (`7d6bb24fc`), `aws-sdk` v2 → per-service `@aws-sdk/client-*` v3 (15 files; 11 new direct deps including S3/RDS/ECS/CodeBuild/CodePipeline/CloudWatchLogs/CloudWatchEvents/Pinpoint/SNS clients + @aws-sdk/s3-request-presigner + @aws-sdk/lib-storage; uniform `client.method(params).promise()` → `client.send(new MethodCommand(params))` conversion; `getSignedUrl` became async; `s3.upload()` → `lib-storage.Upload`; GetObject Body is now a Readable stream — `streamToString` helper in vendor/hls.js; `err.code` → `err.name` for v3 error shape; per-client credentials replace `AWS.config.update()` global mutation) (`8909e8ba5`).
- **What's QUEUED** (in PLAN-recommended order, smallest first):
  ...
  - ~~`sib-api-v3-sdk` (npm-deprecated — was Sendinblue) → `@getbrevo/brevo`~~ ✅ landed as `24599a4ac` — Brevo v5 is a redesigned namespaced API; `vendor/mail.js` rewrote auth (single `new BrevoClient({apiKey})`) + send call (`client.transactionalEmails.sendTransacEmail(...)` with plain-object body). `db/locations.js` had only a commented `// const {Note} = require('sib-api-v3-sdk')`; dropped.
  - ~~`fluent-ffmpeg` (npm-deprecated) → native `child_process.spawn`~~ ✅ landed as `7d6bb24fc` — 1 file (`tasks/video-processing.js`, AMS pipeline). `ffmpeg.ffprobe(path, cb)` → Promise-wrapped `spawn('ffprobe', [-v quiet -print_format json ...])` + JSON.parse stdout; fluent builder → flat CLI args + `spawn('ffmpeg', args)`; `-progress pipe:1` flag for clean key=value progress on stdout (avoids stderr-log parsing fluent-ffmpeg did).
  - ~~`aws-sdk` v2 → v3~~ ✅ landed as `8909e8ba5` — 15 files; per-service `@aws-sdk/client-*` (S3, RDS, ECS, CodeBuild, CodePipeline, CloudWatchLogs, CloudWatchEvents, Pinpoint, SNS) + presigner + lib-storage. Uniform conversion: `client.method(params).promise()` → `client.send(new MethodCommand(params))`; per-client credentials (no `AWS.config.update()`); `getSignedUrl` async; `s3.upload()` → `Upload` from lib-storage; GetObject Body is a Readable stream not a Buffer; `err.code` → `err.name`. SRS code per locked decision #4 is already v3 from day one — this commit covered the rest of cur8-api.
- **Workstreams in progress + not yet started**:
  - **W1 SRS port — Phase 0 + Phase 1 LANDED**: (a) Phase 0 `16313884d` neutral `streaming_streams` table + db layer; (b) Phase 1a `49a25c725` `vendor/srs.js` + `controllers/srs.js` (db swap to `streaming_streams`, aws-sdk v2→v3 swap); (c) Phase 1b `4203d479b` `streaming-service/` container files + `infra/` CDK stack + `docs/srs-fargate-migration-plan.md`. **No AMS code touched** in any of the 3 commits — adheres to locked decision #8 and the 2026-05-17 AMS-infra-hands-off guardrail. **Next phases**: Phase 2 — extend `vendor/ecs.js` with SRS task launch/stop/IP helpers (modifies-existing-file, needs careful AMS-aware merge), wire `controllers/srs.js` routes into `routes/api/*` (needs streaming.provider flag at route layer); Phase 3 — CDK deploy in `cur8-dev` (user-provisioned per locked #1 AWS perms); Phase 4 — RTMP router (the ~1-week long pole, separate Fargate task per locked #3); Phase 5 — tests (the salvage branch had zero; PLAN §W1 #5 lists 6 categories needed).
  - W2 Docker (Dockerfiles + buildspecs + GHA workflows to patch-pinned Node 24; README/CONTRIBUTING/CLAUDE.md yarn→pnpm). After W1 because deploy images need to know about SRS infra.
  - W4 venue builder draft endpoints + adapters (cur8-ui W4 admin UI has a hard dep on this).
- **Resume command from a fresh chat**: `cd ~/Code/cur8-api && git pull && pnpm install` — verify lint + node-check clean. W1 Phase 0 (`16313884d`) + W1 Phase 1 (`49a25c725` + `4203d479b`) + 4 audit fix-ups (`ac30504f3`, `6cd26c6da`, `18f9e120e`, `9dd17811b`) + W1 Phase 2a (`9c2101462` — vendor/ecs.js SRS helpers, AMS code preserved) all landed. **Next: W1 Phase 2b — wire `controllers/srs.js` routes into `routes/api/*`** (`/srs/callbacks/{on-publish,on-unpublish,on-hls,on-play}` for SRS HTTP hooks; `/srs/validate-publish` for the RTMP router contract). Then Phase 2c — add `streaming.provider` flag inspection at the route layer so SRS routes only fire when flag is `srs` (default `ant-media` per locked #2). Then Phase 2d — thread `stream.status` into adaptive URL callers wherever Phase 2 swaps existing flows to SRS (per Codex audit pass 4 / `9dd17811b` contract; getLiveStreamAdaptiveVideoUrl + getFilmReviewAdaptiveVideoUrl require `{status}` or throw). After Phase 2: Phase 3 (CDK deploy in cur8-dev), Phase 4 (RTMP router — the long pole), Phase 5 (tests). This is the first modifies-existing-file SRS work — must preserve the AMS-aware code already in `vendor/ecs.js` (`launchVideoProcessingTask`, `getTask`) and add SRS helpers alongside. Then wire `controllers/srs.js` routes into `routes/api/*` (`/srs/callbacks/{on-publish,on-unpublish,on-hls,on-play}`, `/srs/validate-publish` for the RTMP router contract) — needs `streaming.provider` flag inspection at the route layer so the new routes only fire when the flag is `srs`. After Phase 2: Phase 3 (CDK deploy in cur8-dev — user provisions per locked #1 AWS perms; `cd infra && npm install && cdk synth && cdk deploy`), Phase 4 (RTMP router as a separate always-on Node-proxy Fargate task per locked #3 — the ~1-week long pole), Phase 5 (tests — PLAN §W1 #5 lists 6 categories). All AMS files remain PRESERVED — `terraform/ant-media/`, `controllers/ant-media.js`, `vendor/ant-media.js`, `db/ams-streams.js`, `db/ams-videos.js`, the `ams_streams`/`ams_videos` tables, and the `@antmedia/webrtc_adaptor` on cur8-ui side stay until the post-SRS-prod-validation `chore/ams-removal` follow-up PRs per repo (locked #8 + AMS-infra-hands-off guardrail).

### `showtix4u-venues` — branch `main`

- **HEAD**: `6eb95034` `docs(plan): W1 SRS Phase 1 landed (49a25c725 + 4203d479b)` (pushed; the next commit will be the Codex-audit drift update)
...
- **`pnpm audit`**: **1 moderate vuln** as of 2026-05-17 Codex re-audit: `builder>fabric>jsdom>ws` (GHSA-58qx-3vcg-4xpx; ws <8.20.1, patched ≥8.20.1). Transitive through the venue-builder package's TS deps. Resolution: defer — the builder lives in showtix4u-venues only until W4 cur8-api endpoints land, then it migrates out per locked decision #5 (compile-and-embed in cur8-ui). When it leaves this repo, the vuln chain drops out too. If a fix is needed before W4, bump `fabric` (which would pull a newer jsdom).
...
### 2026-05-17 — cur8-api Wave D-mirror item #5: sib-api-v3-sdk → @getbrevo/brevo

Continues the cur8-api D-mirror queue. 5 of 7 D-mirror replacements landed.

| Hash | LOC | Subject | Notes |
|---|---|---|---|
| `24599a4ac` | 4 files (`vendor/mail.js` real swap + `db/locations.js` 1-line comment delete + `package.json` + lockfile) | `refactor(deps): swap sib-api-v3-sdk → @getbrevo/brevo (Wave D-mirror)` | Drops `sib-api-v3-sdk@8.5.0` (npm-deprecated legacy Sendinblue SDK). Adds `@getbrevo/brevo@5.0.4` (maintained replacement; Sendinblue rebranded to Brevo). **Brevo v5 is NOT a drop-in for the legacy `SibApiV3Sdk` classes** — it's a redesigned namespaced client. Conversion: (a) `SibApiV3Sdk.ApiClient.instance.authentications['api-key']` global-default-client + 4-line setup → single `new BrevoClient({apiKey})` constructor; (b) `new SibApiV3Sdk.TransactionalEmailsApi()` → `new BrevoClient({apiKey}).transactionalEmails`; (c) `new SibApiV3Sdk.SendSmtpEmail()` + property assignments → plain object literal `{to, sender, subject, htmlContent}`; (d) `sendinblue.sendTransacEmail(...)` → `sendinblue.transactionalEmails.sendTransacEmail(...)`. Variable name `sendinblue` retained because the **config namespace** (`sendinblue.api_key`, `sendinblue.dev_whitelist`, etc.) still reflects the old service name and isn't being renamed in this commit. Error-handling block at `vendor/mail.js:333-347` left unchanged — reads `error.message` + `error.code` defensively; `BrevoError` has `.message`, `.code` would have been undefined under sib too. `db/locations.js:3`: dropped commented `// const {Note} = require('sib-api-v3-sdk')` (no real call). Validation: `node --check` on both modified files clean; `node -e "new (require('@getbrevo/brevo').BrevoClient)({apiKey:'fake'}).transactionalEmails.sendTransacEmail"` resolves to a function; `pnpm lint` 0 errors / 22 warnings (unchanged); `pnpm format:check` green; `pnpm audit` 4 vulns (unchanged — sib's transitive `superagent>qs` issues drop out of the tree but mocha vulns remain dominant). |

**Still queued on cur8-api** (2 of 7 D-mirror items remain):

- **NEXT** `fluent-ffmpeg` (npm-deprecated) → audit usage first; 1 file (`tasks/video-processing.js`); server-side so native `child_process` spawn of ffmpeg is the obvious pick over `@ffmpeg/ffmpeg` (browser-side).
- `aws-sdk` v2 → v3 (15+ sites; biggest scope; do last). Per-service `@aws-sdk/client-*` imports + new client/command pattern. SRS code that lands in W1 is already written against v3 per locked decision #4 / #16 — this task converts the **rest** of cur8-api.

After Wave D-mirror queue clears: **W1 SRS** — the original critical path, lands on top of the modernized dep surface. Then W2 Docker. Then W4 venue endpoints.

### 2026-05-17 — cur8-api Wave D-mirror item #6: fluent-ffmpeg → child_process.spawn

Continues the cur8-api D-mirror queue. 6 of 7 D-mirror replacements landed. Only aws-sdk v2→v3 remains, and it's the biggest.

| Hash | LOC | Subject | Notes |
|---|---|---|---|
| `7d6bb24fc` | 3 files (`tasks/video-processing.js` real rewrite + `package.json` + lockfile); 139+/86- | `refactor(deps): replace fluent-ffmpeg with child_process spawn (Wave D-mirror)` | Drops `fluent-ffmpeg@2.1.3` (npm-deprecated wrapper around spawn'd ffmpeg/ffprobe). Only call site: `tasks/video-processing.js`, the AMS video-processing pipeline (`ams_videos` table, `ant_media.s3_bucket` config). File stays in-tree until the post-SRS AMS-removal follow-up PR per locked decision #8. **Replacement strategy**: native `child_process.spawn` of `ffmpeg` + `ffprobe` binaries (same binary requirement as before — fluent-ffmpeg didn't bundle either). Specifics: (a) new `ffprobe(path)` Promise helper that spawns `ffprobe -v quiet -print_format json -show_format -show_streams` and JSON.parses stdout — used by both `checkAudio` + `getFrameRate`; (b) `getFrameRate` now also captures `durationSeconds` from `metadata.format.duration` so the progress-percent calculation has it without a third ffprobe call; (c) the `ffmpeg().input(...).complexFilter(...).outputOptions(...).output(...)` fluent builder becomes a plain CLI args array passed to `spawn('ffmpeg', args)`; (d) **`-progress pipe:1` flag** added to args — ffmpeg emits clean `key=value\n` progress blocks to stdout (separated by `progress=continue` / `progress=end`), much cleaner than parsing the verbose stderr log fluent-ffmpeg relied on; (e) `run()` parses progress blocks as they arrive, extracts `out_time_us` (microseconds), computes percent = `us / 1_000_000 / durationSeconds * 100` (was: `progress.percent` from fluent-ffmpeg). Db update throttle preserved (every 5s); log codes preserved (`ffmpeg-start`, `ffmpeg-progress`, `ffmpeg-error`, `ffmpeg-complete`); update shape preserved (`ams_videos.update({id, framerate, ffmpeg_progress_percent})`). Validation: `node --check`: clean; progress-block parser smoke test (`frame=42\nfps=24\nout_time_us=4163000\nprogress=continue\n` → percent correct); `pnpm lint` 0 errors / 22 warnings (unchanged); `pnpm format` ran format-write to expand the spawn args arrays onto multiple lines per oxfmt, then `format:check` green. |

**Still queued on cur8-api** (1 of 7 D-mirror items remains):

- **NEXT** `aws-sdk` v2 → v3 (15+ sites; biggest scope; last by design). Per-service `@aws-sdk/client-*` imports (s3, ses, sns, etc.) + new client/command pattern. SRS code that lands in W1 is already v3 per locked decision #4 / #16 — this converts the rest of cur8-api. **After this lands, Wave D-mirror queue is complete and W1 SRS — the original critical path — begins.**

### 2026-05-17 — cur8-api Wave D-mirror item #7 (aws-sdk v2 → v3); Wave D-mirror COMPLETE

Final D-mirror replacement. All 7 items landed across the past 2 days. cur8-api dependency modernization is done; next active workstream is **W1 SRS** — the original critical path.

| Hash | LOC | Subject | Notes |
|---|---|---|---|
| `8909e8ba5` | 17 files (15 v2 import sites converted + `package.json` + `pnpm-lock.yaml`); 750+/462- | `refactor(deps): migrate aws-sdk v2 → v3 per-service clients (Wave D-mirror)` | Drops `aws-sdk@2.1693.0` (officially deprecated by AWS; only critical CVEs being patched until end-of-life). 11 new direct deps (per actually-used service): `@aws-sdk/client-s3`, `@aws-sdk/s3-request-presigner`, `@aws-sdk/lib-storage`, `@aws-sdk/client-rds`, `@aws-sdk/client-codebuild`, `@aws-sdk/client-cloudwatch-logs`, `@aws-sdk/client-codepipeline`, `@aws-sdk/client-ecs`, `@aws-sdk/client-pinpoint`, `@aws-sdk/client-sns`, `@aws-sdk/client-cloudwatch-events`. **15 files converted** — every aws-sdk import site in the codebase: `vendor/{s3,hls,ecs,sms,codepipeline}.js`, `vendor/exp/aws.js`, `controllers/uploads.js`, `tasks/{video-processing,codebuild-alerts,sitemap-robots-update,request-log-insights,vimeo-stream-cleanup}.js`, `tasks/dev/s3-prefix-copy.js`, `tasks/devops/share-prod-snapshot.js`, `db/migrations/legacy/2018-11-20/upload-posters-ads.js`. **Conversion pattern (uniform)**: (a) `new AWS.X({apiVersion, accessKeyId, secretAccessKey, region})` → `new XClient({region, credentials: {accessKeyId, secretAccessKey}})`; (b) `client.method(params).promise()` → `client.send(new MethodCommand(params))`; (c) `AWS.config.update(...)` (global mutation, v2-only) → per-client credentials in each client constructor (v3 has no global config); (d) `err.code === 'X'` → `err.name === 'X'` for v3 error shape; also handles `NoSuchKey` for S3 404s (`vendor/s3.js`). **Non-mechanical conversions worth flagging**: (i) v3's `getSignedUrl(client, command, options)` is async (was sync `.getSignedUrl(op, params)` in v2); the `.map()` over m3u8 segment lines in `vendor/hls.js` became `Promise.all(lines.map(async ...))`; `Expires` field renamed to `expiresIn`; (ii) v2's `s3.upload(params).promise()` multipart helper is gone in v3 — `@aws-sdk/lib-storage`'s `Upload` class is the equivalent (`tasks/video-processing.js`); (iii) v3 `GetObjectCommand` response Body is a `Readable` stream (was a `Buffer` in v2); added `streamToString` helper in `vendor/hls.js` for collecting to a string, and in `tasks/video-processing.js` piped directly to `fs.createWriteStream`; (iv) v3 `result.tasks[0]` directly (was `result.data.tasks[0]` envelope in v2 — `vendor/ecs.js`); (v) `tasks/vimeo-stream-cleanup.js` only called `AWS.config.update()` with no actual service usage — dead config update; `require('aws-sdk')` deleted entirely from that file. SRS code per locked decision #4 is already `@aws-sdk/*` v3 from day one — this commit converts only the non-SRS surface. Validation: 9 v3 clients smoke-test (each constructs with fake creds and exposes `.send()`); presigner + Upload resolve to functions; `node --check` clean on all 15 modified files; `pnpm lint` 0 errors / 22 warnings (unchanged baseline); `pnpm format` ran format-write (oxfmt expanded several multi-key require destructures), `format:check` now green; `pnpm audit` 4 → 3 vulns (aws-sdk's low region-param validation cleared; 3 remaining are all mocha transitive). |

**Wave D-mirror queue: COMPLETE (7 of 7).** Summary:

| # | Item | Commit |
|---|---|---|
| 1 | `randomized-string` → `crypto.randomBytes` | `3ec996ffd` |
| 2 | `promise-mysql` + legacy `mysql` → `mysql2` | `959f06bd1` |
| 3 | `@google/maps` → `@googlemaps/google-maps-services-js` | `457ad59f5` |
| 4 | `moment` + `moment-timezone` → `dayjs` | `9eec44fa8` |
| 5 | `sib-api-v3-sdk` → `@getbrevo/brevo` | `24599a4ac` |
| 6 | `fluent-ffmpeg` → `child_process.spawn` | `7d6bb24fc` |
| 7 | `aws-sdk` v2 → per-service `@aws-sdk/*` v3 | `8909e8ba5` |

**Program-wide audit numbers (cur8-api):**

| Phase | Vulns | Notes |
|---|---|---|
| W0 baseline (2026-05-14) | 105 (8c/33h/40m/24l + 8l) | — |
| post-Wave-A+B+C bump | 4 (0c/1h/1m/2l) | 96% reduction; aws-sdk low + mocha vulns |
| **post-Wave-D-mirror complete (now)** | **3 (0c/1h/1m/1l)** | **97% reduction**; only mocha transitive remains |

**Active workstream now: W1 SRS port.** The original critical path. ~1.5–2 weeks; RTMP router (always-on Node-proxy Fargate task) is the long pole — ~1 week alone. CDK infra (locked #1, no terraform). Neutral `streaming_streams` table (locked #2 + W1 §7). Single global `streaming.provider` flag (locked #2). SRS code already written against `@aws-sdk/*` v3 from day one (locked #4). User has AWS perms on `cur8-dev` for Fargate / ECS / EKS provisioning (locked #1). Tests for everything (the old branch had zero — full list at W1 §5). See §"Workstream 1" for the full task breakdown. After W1 lands: W2 Docker (both repos) → W4 venue endpoints (cur8-api) → W4 venue admin UI (cur8-ui) → post-SRS-prod-validation AMS-removal follow-up PRs (one per repo, locked #8).

### 2026-05-17 — W1 SRS Phase 0 + audit decision + AMS-hands-off guardrail recorded

After Wave D-mirror complete (previous entry), session continued into W1 SRS prep. Approach split into phases so infra-independent prep can land before CDK provisioning is needed.

**Audit-to-zero decision (no overrides):** Initial attempt to drive cur8-api audit from 3 → 0 used `pnpm.overrides` in `package.json` (`serialize-javascript@<7.0.5: >=7.0.5`, `diff@<8.0.3: >=8.0.3`). User reverted: "we ideally shouldn't have ANY overrides. and now with pnpm 11, you put overrides in the workspace." Decision recorded:

- **No `pnpm.overrides` for mocha vulns.** mocha is at latest (11.7.5); no upstream fix available. Override would mask the vulnerability rather than fix it — wrong direction. Accept the 3 vulns (1 high RCE + 1 moderate DoS from `serialize-javascript`, 1 low DoS from `diff`); document in cur8-api snapshot.
- **Blast radius:** mocha is a devDep (test runner). Vulns don't manifest at production runtime. Re-evaluate when mocha 12 lands or if any mocha bump picks up newer transitive deps.
- **Per-user-memory** ([[feedback-no-overrides]]): if an override ever becomes unavoidable, pnpm 11+ puts it in `pnpm-workspace.yaml` (the canonical pnpm 11 location), not in `package.json`. Surface the trade-off and ask before adding.

**AMS-infra-hands-off guardrail recorded.** User explicit guidance 2026-05-17: "MAKE SURE you don't touch the ant media server infrastructure. I will take that down when I'm ready." Captured as [[feedback-ams-infra-hands-off]]. The no-touch list during the SRS port phase:

- `terraform/ant-media/` and the AMS Fargate / EC2 / NLB AWS resources
- `controllers/ant-media.js`, `vendor/ant-media.js`
- `db/ams-streams.js`, `db/ams-videos.js`, and the `ams_streams` + `ams_videos` MySQL tables
- `tasks/scheduled-streams.js`'s AMS code paths (modify carefully if at all)
- The `streaming.provider` global config flag default (stays `ant-media` until the dev→staging→prod flip sequence in W1)
- cur8-ui: `@antmedia/webrtc_adaptor`, `containers/Demo/AntMediaPage`, `containers/CUR8Support/AMS`, AMS-specific code in `containers/CUR8Support/StreamingList`

The AMS removal happens via the post-SRS-prod-validation `chore/ams-removal` follow-up PRs per repo (locked decision #8). Premature AMS deletion forfeits the rollback path.

**Salvage survey of `feature/streaming-service` (2026-05-17, head `5c6dccf00`):**

NEW SRS-only files (the easy port):
- `controllers/srs.js` — HTTP callback handlers (`on_publish`, `on_unpublish`, `on_hls`, `on_play`) + a presigned-HLS proxy. Uses `aws-sdk@2` on the branch; needs the v3 swap during port (locked decision #4). ~callback secret validation, ~stream lookup, ~status updates.
- `vendor/srs.js` — token gen/validate (`getStreamPublishToken` SHA-256), `generateStreamId`, `createLiveEvent`. No AWS SDK in this file; pure crypto + db. Drop-in port.
- `streaming-service/Dockerfile` + `entrypoint.sh` + `srs.conf` + `docker-compose.yml` + `README.md` — the SRS container itself. Built by the CDK stack, deployed to ECR by the user via `cur8-dev` perms.
- `infra/lib/srs-stack.ts` — CDK stack: ECR repo + CloudWatch log group + scoped IAM task role (S3 put + presign only to `SRS/*` prefix) + ECS task definition + custom-resource glue. **No terraform** per locked decision #1.
- `infra/{bin/infra.ts,cdk.json,package.json,tsconfig.json}` — CDK app entry + config + TS toolchain.
- `docs/srs-fargate-migration-plan.md` — full architecture doc: RTMP routing strategy, the 3 use cases (livestream / scheduled / film review), auth flow, S3 upload strategy, HLS serving, cost analysis ($456→$61/mo).

Modified-on-salvage-branch files that need careful re-port (10):
- `controllers/streaming.js`, `controllers/{ant-media,events,library,notifications,communications}.js` — provider-aware routing of stream lifecycle. AMS code must be PRESERVED; SRS code adds parallel branches gated on `streaming.provider`.
- `vendor/{ecs,s3,tus,ant-media}.js` — `ecs.js` gets SRS task launch helpers; `s3.js` gets HLS path support for `SRS/*` prefix; `tus.js` for upload integration; `ant-media.js` PRESERVED.
- `tasks/scheduled-streams.js` — provider-aware scheduled stream task launcher.

The salvage branch's `db/migrations/2026-03/add-task-arn-to-ams-streams.sql` is **NOT ported** per PLAN §W1 #7 — it took the wrong path (extending the going-away table). W1 Phase 0 (this commit) put `task_arn` on the neutral `streaming_streams` table instead.

**Commit landed today (cur8-api W1 Phase 0):**

| Hash | Subject | Files |
|---|---|---|
| `16313884d` | `feat(srs): W1 Phase 0 — neutral streaming_streams table + db layer` | NEW `db/migrations/2026-05/streaming-streams.sql` (neutral table; `provider` + `task_arn` columns added vs ams_streams shape; indexes per W1 §7) + NEW `db/streaming-streams.js` (mirrors all 7 `db/ams-streams.js` exports against the new table). 171+/0-. No AMS files touched. Infra-independent — migration is no-op for AMS-only traffic (starts empty, nothing reads until `streaming.provider` flips to `srs`). |

**Status:**
- cur8-api branch at `16313884d` (W1 Phase 0). **Unpushed at time of this PLAN write** — push happens alongside the PLAN commit so the snapshot and the code land synced.
- W1 Phase 1 (SRS code port from salvage branch) is the next active work. Needs ~1 session per file group. CDK infra (Phase 2) needs user AWS perms in cur8-dev.
- cur8-ui `dev-with-upgrade-2026q2` unchanged — still health-gate clean, still waiting on cur8-api W1 HLS port.
- Wave D-mirror queue: still complete. Audit holds at 3 mocha vulns per the decision above.

### 2026-05-17 — W1 SRS Phase 1 landed (Phase 1a + Phase 1b)

Continues from the same session that closed Wave D-mirror + W1 Phase 0. Phase 1 is "port the NEW SRS files from `feature/streaming-service` (head `313985324`)" — purely additive, zero AMS touch.

**Survey citation correction:** The 2026-05-17 night entry above cited commits like `5c6dccf00 ticket-5790` as being on `feature/streaming-service`. **They aren't** — they're on `staging`, picked up by a sloppy symmetric `...` diff. The actual salvage HEAD is `313985324 "updates to srs"` from **2026-03-30**. The 9-NEW-file inventory was correct; only the commit-ID attribution was off. (User caught this with: "did you check against some of the branches I already had?") Other streaming-* branches surveyed: `feature/CUR8-5057-streaming-monitor`, `streaming-monitor-5494`, etc. — all AMS-era streaming features, none have SRS / `streaming-service/` / `infra/` files. Salvage source is uniquely `feature/streaming-service`.

| Hash | LOC | Subject | Notes |
|---|---|---|---|
| `49a25c725` | 3 files / 856+/0- | `feat(srs): W1 Phase 1a — port vendor/srs.js + controllers/srs.js (v3 SDK)` | (a) `vendor/srs.js` (368L): drop-in port; **db.ams_streams → db.streaming_streams** swap (5 sites + 3 `db.writer('streaming_streams')` calls); no AWS SDK in this file. (b) `controllers/srs.js` (335L): port + db swap (5 sites) + **aws-sdk v2 → v3 swap** per locked #4 — `new AWS.S3({apiVersion, accessKeyId, secretAccessKey, region})` → `new S3Client({region, credentials})`; `s3.listObjectsV2(p).promise()` → `s3.send(new ListObjectsV2Command(p))`; same for putObject. (c) `db/index.js`: register `streaming_streams: require('./streaming-streams')` alongside the preserved `ams_streams` registration. **Forward references** to `ecs.launchSrsStreamTask` / `ecs.getSrsTaskPrivateIp` / `ecs.stopSrsTask` are intentionally undefined on `vendor/ecs.js` until Phase 2 — modules load cleanly via lazy `require('./ecs')` inside function bodies; these only throw at call time, which only fires when `streaming.provider = srs` (currently `ant-media` per locked #2). The salvage's `db/migrations/2026-03/add-task-arn-to-ams-streams.sql` is NOT ported — Phase 0 put `task_arn` on the neutral `streaming_streams` table per PLAN §W1 #7. Validation: `node --check` clean on 3 files; `pnpm lint` 0 errors / 24 warnings (+2 from new SRS files — unused-param Express middleware conventions, acceptable); `pnpm format` ran format-write (oxfmt expanded chained .where().update()); `format:check` green. |
| `4203d479b` | 13 files (12 NEW from salvage + 1 modified `.oxfmtrc.json`) | `feat(srs): W1 Phase 1b — streaming-service container + infra CDK + docs` | (a) `streaming-service/` 5 files: `Dockerfile` (builds SRS server image), `entrypoint.sh` (250L — SRS boot + inotifywait-watches /tmp/hls/ + uploads segments to S3 real-time), `srs.conf` (110L — SRS server config: HTTP callbacks to cur8-api, HLS settings, RTMP listener, transcoding hooks), `docker-compose.yml`, `README.md`. (b) `infra/` 6 files: `lib/srs-stack.ts` (224L — canonical CDK stack: ECR repo with lifecycle keep-last-10 + RETAIN removal, CloudWatch log group, scoped IAM task role NOT using shared ecsTaskExecutionRole — task role limits S3 to `SRS/*` prefix only for put/get/presign, ListBucket scoped to bucket+prefix), `bin/infra.ts` (CDK app entry), `cdk.json` + `tsconfig.json` + `package.json` (CDK toolchain), `README.md`, plus NEW `.gitignore` (CDK build outputs: node_modules, cdk.out, *.d.ts, compiled *.js — prevents CDK build artifacts polluting git when user runs `cdk synth`). (c) `docs/srs-fargate-migration-plan.md` (223L — architecture doc: AMS-vs-SRS cost analysis $456→$61/mo, 3 use cases livestream/scheduled/film-review, RTMP routing strategy with always-on Node-proxy task behind NLB + Redis streamId→taskPrivateIp lookup, auth flow with SHA-256 publish token + cur8-api session auth on HLS proxy routes, S3 upload via inotifywait background, HLS serving unchanged — cur8-api proxies S3 presigned URLs, just swap `CUR8Live/` prefix → `SRS/`). (d) `.oxfmtrc.json` modified — added 3 `ignorePatterns`: `infra/**` (TS/JSON/MD with CDK ecosystem conventions, oxfmt's reformat was 24-330 lines per file with no semantic benefit), `streaming-service/**` (Dockerfile/yaml/sh/conf with their own formatting; README is reference doc), `docs/srs-fargate-migration-plan.md` (authoritative architecture doc, preserve verbatim). Validation: all 3 infra JSON files parse; `bash -n streaming-service/entrypoint.sh` clean; `pnpm lint` 0 errors / 24 warnings (unchanged); `pnpm format:check` green; **no AMS files touched** (confirmed via git status). |

**What W1 Phase 1 does NOT include** (deferred to subsequent phases):

- **Phase 2 — wiring**: extending `vendor/ecs.js` with SRS task helpers (`launchSrsStreamTask`, `getSrsTaskPrivateIp`, `stopSrsTask`) alongside the AMS-aware `launchVideoProcessingTask` + `getTask`; wiring `controllers/srs.js` routes into `routes/api/*` with `streaming.provider` flag inspection at the route layer.
- **Phase 3 — CDK deploy**: `cd infra && npm install && cdk synth && cdk deploy` in `cur8-dev`. User-provisioned per locked #1 AWS perms.
- **Phase 4 — RTMP router**: separate always-on Node-proxy Fargate task per locked #3. The ~1-week long pole.
- **Phase 5 — tests**: the salvage branch had zero. PLAN §W1 #5 lists the 6 categories (publish token validation, callback auth, lifecycle, S3 paths, scheduled startup, rollback / provider-flag selection).
- **Modifying-existing-file SRS work**: 10 files on the salvage branch had SRS-related modifications (`controllers/{streaming,events,library,notifications,communications}.js`, `vendor/{s3,tus}.js`, `tasks/scheduled-streams.js`, etc.) that integrate SRS via the provider flag. Re-port carefully alongside the AMS code in those files. Phase 2+ work.

**Status:**
- cur8-api `feat/upgrade-2026q2` @ `4203d479b` — pushed. Wave D-mirror complete + W1 Phase 0 + W1 Phase 1 all landed.
- cur8-ui `dev-with-upgrade-2026q2` @ `1c4ccf0d1` — unchanged. Still health-gate clean. W1 HLS port (cur8-ui side) blocked on cur8-api Phase 2+ surface stabilizing.
- showtix4u-venues `main` — about to receive this PLAN.md update.

### 2026-05-17 — Codex post-Phase-1 audit: drift + 3 Phase 1 findings

After W1 Phase 1 landed (entries above), session paused. Codex ran an independent post-port audit and surfaced both branch-state drift and 3 SRS Phase 1 issues worth fixing before Phase 2 wiring begins.

**Branch-state drift (all snapshots updated above):**

- **cur8-ui** behind `origin/dev`: 5 → **9** commits (dev moved forward 4 more during the pause).
- **cur8-ui** `pnpm lint`: prior PLAN claimed "23 warnings" — that was actually the "23 fixable" subcount being misread as total. **Actual count: 195 warnings (0 errors).** 23 of the 195 are `--fix`-able; the rest are dead-code / unused-var nits inherited from pre-upgrade code. Cleanup optional.
- **cur8-api** `pnpm outdated`: previously "0 deprecated" was true and still is, but **3 patch-level updates available**: `qs` 6.15.1→6.15.2, `oxlint` 1.64.0→1.65.0, `oxfmt` 0.49.0→0.50.0. Sweep before any merge.
- **cur8-api PR #3630**: 396 → **422** changed files (staging diverged further). State at last check: DIRTY (CONFLICTING). Hold per existing decision until SRS wiring + Docker ready. As of the 2026-05-18 local-only review instruction, **do not re-query PR state during ordinary audits**; use local git + health gates unless the user asks for GitHub/PR status.
- **showtix4u-venues** `pnpm audit`: now **1 moderate** (was 0): `builder>fabric>jsdom>ws` (GHSA-58qx-3vcg-4xpx; ws <8.20.1 needs ≥8.20.1). Builder migrates out of this repo at W4 per locked #5 — vuln chain drops then. If urgent, bump `fabric`.

**3 SRS Phase 1 findings (queued as the next 3 fixes before Phase 2):**

| Sev | Finding | Detail | Fix |
|---|---|---|---|
| **HIGH** | HLS naming inconsistency between container and API | `streaming-service/entrypoint.sh:99` writes `${STREAM_ID}_adaptive.m3u8` to disk + uploads to S3 with that name. `vendor/srs.js:256/263/271` (getS3MasterPlaylistKey, getLiveStreamAdaptiveVideoUrl, getFilmReviewAdaptiveVideoUrl) read `SRS/${streamId}/${streamId}_master.m3u8`. `controllers/srs.js buildVodArchive` also writes `${streamId}_vod_master.m3u8`. Once Phase 2 wires this, playback will 404 — wrong file name in S3. | Standardize on `${streamId}_master.m3u8` (matches the JS code + buildVodArchive convention). Change one line in entrypoint.sh. |
| MEDIUM | `createLiveEvent` shape mismatch with AMS contract | `vendor/srs.js:91` does `const internal_id = await db.streaming_streams.insert(record)` but `insert()` returns the WHOLE inserted row (see `db/streaming-streams.js:56` — mirrors `db/ams-streams.js` AMS contract for compat). Then line 122 returns that row AS the `internal_id` field on the live event. Caller expects an int. | Rename var to `streamRecord`; extract `.internal_id` properly into the returned shape. Same bug shape in `createFilmReviewStream` + `createScheduledBroadcasts` — audit those too. |
| MEDIUM | `entrypoint.sh` cleanup-on-exit | Line 67 trap is `cleanup SIGTERM SIGINT` — only handles signals, NOT non-zero exits. Line 234 `wait $SRS_PID` + `set -e` (top of file) means if SRS exits non-zero, `wait` returns non-zero, `set -e` exits the script BEFORE the `EXIT_CODE=$?` line and the explicit final-sync block at 241-250 can run. Tail HLS files lost on SRS crash. | Add `EXIT` to the trap (`trap cleanup EXIT SIGTERM SIGINT`). Remove the duplicate sync block at 241-250 (cleanup() already does identical work). Also drop the `exit 0` at end of cleanup so SRS's real exit code propagates to ECS. |

LOW / expected finding (also noted): SRS Phase 1 is **additive, not operational**. No route registration yet, no `vendor/ecs.js` SRS helpers, no `streaming.provider` flag inspection at the route layer. That's the Phase 2 scope; flag here so future readers don't mistake "landed" for "usable".

**Session ordering after this PLAN drift commit:** fix the 3 SRS Phase 1 findings as one cohesive commit (or two: HIGH alone then the two MEDIUMs together), THEN proceed to W1 Phase 2 (vendor/ecs.js helpers + route wiring + `streaming.provider` boundary). Avoids cementing shape mismatches into Phase 2 integration. The `pnpm up` patch sweep, cur8-ui `--fix` lint pass, and venues `fabric` bump can be opportunistic/deferred.

### 2026-05-17 — Phase 1 audit fixes landed (ac30504f3)

All 3 findings from the Codex audit (previous entry) fixed in a single cohesive commit. Phase 1 surface stays additive + non-operational; only the shapes change.

| Hash | Files | Subject |
|---|---|---|
| `ac30504f3` | `streaming-service/entrypoint.sh` (29+/18-), `vendor/srs.js` (8+/4-); 2 files | `fix(srs): address 3 Phase 1 audit findings before Phase 2 wiring` |

**HIGH — HLS naming**: `streaming-service/entrypoint.sh` line 99 changed from `${STREAM_ID}_adaptive.m3u8` → `${STREAM_ID}_master.m3u8`. Aligns with `vendor/srs.js` `getS3MasterPlaylistKey`/`getLiveStreamAdaptiveVideoUrl`/`getFilmReviewAdaptiveVideoUrl` (all expect `${streamId}_master.m3u8` in S3) + `controllers/srs.js buildVodArchive` convention. Comment added explaining the alignment.

**MEDIUM — createLiveEvent shape**: `vendor/srs.js` `createLiveEvent`: renamed `internal_id` variable → `streamRecord` (it's the whole inserted row, mirroring AMS interface contract). Returned-shape `internal_id` field now extracts `streamRecord.internal_id` properly (was returning the whole row). Audited `createFilmReviewStream` and `createScheduledBroadcasts` — they discard the `insert()` return, no shape bug there.

**MEDIUM — entrypoint.sh cleanup-on-exit**: `streaming-service/entrypoint.sh`:
- Trap extended from `cleanup SIGTERM SIGINT` → `cleanup EXIT SIGTERM SIGINT`. EXIT fires on any exit (normal, set-e error, signal-after-trap), so final S3 sync always runs even on SRS crash via `wait $SRS_PID` + `set -e`.
- Added `CLEANUP_RAN` guard so SIGTERM-then-EXIT doesn't run cleanup twice.
- Removed `exit 0` at end of cleanup() — was masking SRS crashes as ECS task success. Script's natural exit code now propagates.
- Removed the redundant "final sync" block at the bottom of the script (cleanup() does identical work via the EXIT trap; duplicate would have re-uploaded files after cleanup ran).
- Bottom of script now: `wait $SRS_PID; EXIT_CODE=$?; echo ...; exit $EXIT_CODE` — clean exit-code propagation.

LOW / expected finding (Phase 1 is additive-not-operational) is unchanged — Phase 2 scope.

**Validation:** `bash -n streaming-service/entrypoint.sh` clean; `node --check` clean on both modified JS files; `pnpm lint` 0 errors / 24 warnings (unchanged baseline); `pnpm format:check` green; no AMS files touched.

**Status:** cur8-api `feat/upgrade-2026q2` @ `ac30504f3` — pushed. Wave D-mirror COMPLETE + W1 Phase 0 + W1 Phase 1 + Phase 1 audit fixes ALL landed. **W1 Phase 2 (the next thing) is unblocked** — Phase 1 shapes are now correct for integration.

### 2026-05-18 — local-only review scope for PR state

User clarified during the next Codex review that PR metadata is not useful right now and should not be re-checked during ordinary state-of-the-union audits. Until the user explicitly asks for PR/GitHub status, future reviews should stay local-only:

- `git status -sb`
- branch divergence against the tracked base branch
- local build/lint/format/audit/outdated checks
- source inspection against PLAN.md claims

PR #3630's last known state remains recorded above for context, but it is intentionally stale until merge planning resumes. Do not burn time refreshing it while W1 Phase 2 / W2 Docker are still in progress.

### 2026-05-18 — Phase 2 readiness fix-ups (6cd26c6da)

Codex did a second local-only post-port audit and surfaced 5 more issues — all in the SRS Phase 1 surface, none in AMS. All 5 fixed in one cohesive commit so Phase 2 starts from a clean Phase 1.

| Hash | Files | Subject |
|---|---|---|
| `6cd26c6da` | 5 files (vendor/srs.js, controllers/srs.js, streaming-service/README.md, infra/README.md, infra/package.json); 264+/281- | `fix(srs): Phase 2 readiness — config safety, ready-flag, VOD helper, infra/, READMEs` |

**HIGH — Import-time `config.get()` crashes**:
- `vendor/srs.js` lines 25/31 + `controllers/srs.js` lines 23/37 read `streaming.*` config at module load. `config.get()` throws on missing key BEFORE any `||` fallback evaluates (node-config doesn't short-circuit).
- Currently safe because SRS modules aren't required at runtime (no routes wired yet, no Phase 2). But Phase 2 wiring would crash on first import if `streaming.*` isn't set in dev/staging/prod.
- Fix: `getOr(key, fallback)` helper using `config.has()` guard for non-critical values; security-critical secrets (publish/callback token) read as null at module top and throw lazily at call time via `requireConfig(key, value)`. `controllers/srs.js` `validateCallback` 403s if secret missing rather than letting unauthenticated SRS traffic through. `s3Streaming` client became a lazy builder — falls back to ambient IAM/env credentials (S3Client native behavior) if `streaming.aws_key/secret` aren't set.

**MEDIUM — `createLiveEvent` + `createFilmReviewStream` return shape**:
- Both returned success-shaped data even when Fargate launch failed (the only signal was `task_arn: null` — implicit).
- Phase 2 callers shouldn't need to detect "is task_arn null?" to know the stream is degraded.
- Fix: explicit `task_ready: !!taskInfo` field on `createLiveEvent`; `task_ready: !!taskInfo && !!taskIp` on `createFilmReviewStream` (which needs both task launched + IP resolved for the WHIP endpoint). Comment documents the degraded-mode retry contract.

**MEDIUM-LOW — VOD vs live playlist naming convention**:
- Live writes `${streamId}_master.m3u8`; VOD writes `${streamId}_vod_master.m3u8`. Intentional divergence but undocumented.
- Phase 2 routes must pick correctly based on `stream.status` — asking for live master post-finalize will 404; asking for VOD mid-stream will 404.
- Fix: added `exports.getS3VodMasterPlaylistKey` helper alongside the existing live helper; doc block above both explains the convention, lifecycle, and the routing rule (`broadcasting`/`disconnected` → live; `finished` → VOD).

**LOW — `infra/package.json` packageManager alignment**:
- Was missing `packageManager` field — inconsistent with cur8-api root (`pnpm@11.1.2` via Corepack).
- Fix: added `"packageManager": "pnpm@11.1.2"`. No lockfile generated yet — user generates when they actually run `pnpm install` in `infra/`.

**MEDIUM — Stale READMEs**:
- `streaming-service/README.md` and `infra/README.md` were duplicates of each other from the salvage branch, with 10+ false claims: "vendor/ant-media.js DELETED", "controllers/ant-media.js DELETED", "controllers/streaming.js swapped from vendor/ant-media to vendor/srs", "SRS Docker image built and pushed to ECR (583716518461.dkr.ecr...)", "Fargate task definition registered (cur8-srs-stream:1, 8 vCPU / 16GB)", "Security group sg-0be1068c517f5ba96 created", "Vegas dev API deployed with SRS code". **None true on this branch.** Risk: future readers see "DELETED" and act on it, breaking the AMS-infra-hands-off guardrail.
- Fix: full rewrite of both. `streaming-service/README.md` now scoped to the container only — what each file does, env vars, local-test commands, naming conventions, what's NOT here. `infra/README.md` scoped to the CDK stack — what it provisions, CDK context inputs, deploy commands, IAM scoping rationale (scoped TaskRole rather than shared `ecsTaskExecutionRole`), what's NOT in this stack. Both lead with "Status: PORTED, NOT YET WIRED OR DEPLOYED" + reference [[feedback-ams-infra-hands-off]]. Both end with "What previously appeared here and was wrong" enumerating the false claims so future readers don't get burned.

**Validation:**
- `node --check` on vendor/srs.js + controllers/srs.js: clean
- `bash -n streaming-service/entrypoint.sh`: clean
- `pnpm lint`: 0 errors / 24 warnings (back to baseline after dropping one stray unused-var introduced by the helper refactor)
- `pnpm format:check`: green
- All `config.get('streaming.*')` calls are now either guarded by `config.has()` at module top OR inside function bodies (lazy)
- No AMS files touched

**Status:**
- cur8-api `feat/upgrade-2026q2` @ `6cd26c6da` — pushed. Phase 1 surface now genuinely Phase-2-ready (import-safe, ready-flagged, VOD-routable, packageManager-aligned, honestly documented).
- Phase 2 (vendor/ecs.js SRS helpers + route wiring + streaming.provider boundary) is unblocked. First modifies-existing-file SRS work — must preserve AMS-aware code per the guardrail.

### 2026-05-18 — SRS config-consistency follow-ups (18f9e120e)

Codex did a third post-port audit and surfaced 2 more consistency issues from the Phase 2-readiness commit (`6cd26c6da`). Both followed the same pattern: a config read was guarded at module top but the downstream USE site wasn't.

| Hash | Files | Subject |
|---|---|---|
| `18f9e120e` | 2 files (`vendor/srs.js`, `controllers/srs.js`); 16+/4- | `fix(srs): consistency follow-ups from Codex 2026-05-18 audit` |

**MEDIUM — host URL helpers could emit `null/api/...`**: Phase 2 readiness made `host` import-safe via `getOr('host', null)`, but the 3 URL helpers (`getLiveStreamHlsUrl`, `getFilmReviewHlsUrl`, `getScheduledContentPlaybackUrls`) interpolated `${host}` directly. If a deploy had `streaming.*` configured but `host` missing, stream creation would return malformed URLs like `null/api/streams/...` — the frontend would hit nameservers with `null` as a hostname. Fix: new `getHost()` accessor wraps `host` in `requireConfig('host', host)` so it throws at call time with the `streaming-config-missing` ApiError. All 4 sites converted from `${host}` to `${getHost()}`.

**MEDIUM-LOW — bucket config inconsistent**: `vendor/srs.js` had `getOr('streaming.s3_bucket', 'ant-media.cur8.com')` (guarded with historical fallback). `controllers/srs.js buildVodArchive` line 292 had a bare `config.get('streaming.s3_bucket')` (would throw on missing key, no fallback). Fix: switched buildVodArchive to the same `getOr` pattern. Historical fallback IS the intended behavior (shared infra bucket stays after AMS removal); consistency means future readers see one pattern.

**Validation:** node --check + pnpm lint (0 errors / 24 warnings unchanged) + pnpm format:check all green. Grep confirms 0 bare `${host}/api` patterns remain; 4 `getHost()` call sites; bucket read uses `getOr` with fallback.

**Status:** cur8-api `feat/upgrade-2026q2` @ `18f9e120e` — pushed. Phase 1 surface should now be free of BOTH import-time crashes (Phase 2-readiness commit) AND silent-degradation failure modes (this commit) when Phase 2 wiring fires. Phase 2 (vendor/ecs.js SRS helpers + route wiring + streaming.provider boundary) remains the next active work.

### 2026-05-18 — adaptive-video helpers made status-aware (9dd17811b)

Codex audit pass 4 surfaced one more behavioral consistency issue from the W1 Phase 1 surface. The previous commit (`18f9e120e`) added `getS3VodMasterPlaylistKey` and documented the live-vs-VOD convention, but the higher-level URL helpers stayed status-blind.

| Hash | Files | Subject |
|---|---|---|
| `9dd17811b` | 1 file (`vendor/srs.js`); 51+/6- | `fix(srs): make adaptive-video URL helpers status-aware (require {status})` |

**MEDIUM — `getLiveStreamAdaptiveVideoUrl` + `getFilmReviewAdaptiveVideoUrl` were status-blind**: Both helpers hardcoded `${streamId}_master.m3u8` (the LIVE master key). Phase 2 wiring would have silently pointed viewers at the live rolling playlist for completed/finalized streams — a 404 once the live playlist is gone, even though the VOD playlist exists. AMS call sites at `controllers/library.js:51` + `controllers/notifications.js:53` (preserved per the AMS-infra-hands-off guardrail) are the future Phase 2 swap targets; a status-blind swap would carry the bug.

**Fix:** Both helpers now require `{status}` as a second argument:
- `status === 'finished'` → `getS3VodMasterPlaylistKey(streamId)` (VOD master from buildVodArchive)
- Any other status (`broadcasting` / `disconnected` / `idle` / `created`) → `getS3MasterPlaylistKey(streamId)` (live rolling master)
- Missing status throws `streaming-stream-status-required` ApiError with helper name in context for grep-ability

Throwing on missing status follows the same fail-loud posture as the `host` and `secret` guards in `18f9e120e` — silent default would 404 viewers on completed streams AND silently 404 in the other direction (asking for VOD mid-stream). Phase 2 callers must look up `stream.status` from `db.streaming_streams.get` and pass it through.

JSDoc on both helpers explains the status mapping, why status is required, and references `db.streaming_streams.get` as the canonical source for the value.

**Validation:** node --check clean; pnpm lint 0 errors / 24 warnings (unchanged); pnpm format:check green; inline smoke confirms throws on missing status + correct key for each status (broadcasting/disconnected → live, finished → VOD).

**Status:** cur8-api `feat/upgrade-2026q2` @ `9dd17811b` — pushed. Phase 1 surface should now be free of (a) import-time crashes (`6cd26c6da`), (b) silent-degradation failure modes (`18f9e120e`), and (c) status-blind URL routing (`9dd17811b`). All known Codex post-port audit findings closed. Phase 2 (vendor/ecs.js SRS helpers + route wiring + streaming.provider boundary) remains the next active work.

### 2026-05-18 — W1 SRS Phase 2a landed: vendor/ecs.js SRS helpers (9c2101462)

First **modifies-existing-file** SRS work. Pure-additive port of 3 SRS task helpers into `vendor/ecs.js` alongside the preserved AMS-aware `launchVideoProcessingTask` + `getTask`. Zero changes to existing AMS code paths per the AMS-infra-hands-off guardrail.

| Hash | Files | Subject |
|---|---|---|
| `9c2101462` | 1 file (`vendor/ecs.js`); 176+/1- | `feat(srs): W1 Phase 2a — port SRS ECS helpers into vendor/ecs.js (preserve AMS)` |

**3 new exports (additive):**

- `launchSrsStreamTask({streamId, streamType, publishToken, sourceM3u8Url, startTime})` — Fargate task launch. Builds the env-var array consumed by `streaming-service/entrypoint.sh` (STREAM_ID, STREAM_TYPE, S3_BUCKET, S3_PREFIX, PUBLISH_TOKEN, API_CALLBACK_URL, SRS_CALLBACK_SECRET; plus SOURCE_M3U8_URL + START_TIME for scheduled-content). Returns `{taskArn, taskId}`. Throws on launch failure with the Fargate failure reason in the error message.
- `getSrsTaskPrivateIp({taskArn, cluster, maxWaitMs, pollIntervalMs})` — polls `DescribeTasks` until the task's ENI has a `privateIPv4Address` detail. Used by the RTMP router (Phase 4) for `streamId → taskPrivateIp` Redis routing per locked decision #3. Defaults: maxWaitMs=60000, pollIntervalMs=3000.
- `stopSrsTask({taskArn, cluster, reason})` — wraps `StopTaskCommand`. Called by `controllers/srs.js finalizeIdleStream` after the 30-min idle timeout.

**Conversion notes (from salvage @ 313985324):**

- aws-sdk v2 → v3 per locked decision #4: `ecs.runTask(p).promise()` → `ecs.send(new RunTaskCommand(p))`; same for DescribeTasks and StopTask. `StopTaskCommand` added to the existing import.
- All config reads use the new `getOr(key, fallback)` helper — mirrors `vendor/srs.js` posture from `6cd26c6da`. Falls through to existing `aws.cluster` / `aws.subnets` / `aws.security_groups` if SRS-specific overrides aren't set (cluster shared with AMS task launches).
- Logger lazy-required inside each SRS helper to keep the existing `vendor/ecs.js` module-load surface unchanged (the AMS helpers don't use logger).

**AMS preservation verified:**

- `launchVideoProcessingTask` body unchanged
- `getTask` body unchanged
- Only diff in AMS section: import line expanded from one-line to multi-line for the new `StopTaskCommand` addition (pure formatting, oxfmt reformatted)
- AMS code paths fire on the same RunTask/DescribeTask v3 commands as before

**Cross-verification of `vendor/srs.js` call sites against new exports:**

| `vendor/srs.js` call | `vendor/ecs.js` export | Match? |
|---|---|---|
| `ecs.launchSrsStreamTask({streamId, streamType, publishToken})` × 3 sites (createLiveEvent, createFilmReviewStream, createScheduledBroadcast) | `launchSrsStreamTask` | ✅ |
| `ecs.getSrsTaskPrivateIp({taskArn})` × 2 sites (createLiveEvent, createFilmReviewStream) | `getSrsTaskPrivateIp` | ✅ |
| `ecs.stopSrsTask({taskArn, reason})` × 1 site (`controllers/srs.js finalizeIdleStream` via `require('../vendor/ecs')`) | `stopSrsTask` | ✅ |

Previously these calls would have thrown `TypeError: ecs.launchSrsStreamTask is not a function` at first invocation. Now they resolve. (Still don't fire until `streaming.provider === 'srs'` per locked #2.)

**Validation:**

- `node --check vendor/ecs.js`: clean
- `pnpm lint`: 0 errors / 24 warnings (unchanged baseline)
- `pnpm format:check`: green
- AMS-preservation grep: only deletion is the old single-line import (replaced with multi-line incl. `StopTaskCommand`)

**Status:** cur8-api `feat/upgrade-2026q2` @ `9c2101462` — pushed. SRS module surface is now functionally complete (Phase 1) + ECS-wired (Phase 2a). **Phase 2b** (route registration in `routes/api/*`) is the next active work. After 2b: 2c (provider-flag gating), 2d (status-threading in adaptive URL callers wherever Phase 2 swaps existing flows). Then Phase 3 (CDK deploy), Phase 4 (RTMP router, the long pole), Phase 5 (tests).
