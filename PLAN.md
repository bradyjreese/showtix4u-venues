...
- **HEAD**: `7d6bb24fc` `refactor(deps): replace fluent-ffmpeg with child_process spawn (Wave D-mirror)` (pushed to `origin/feat/upgrade-2026q2`)
- **Base**: `staging`
- **Build**: lint clean (0 errors, 22 warnings — unchanged through 6 dep-replacement commits); `node --check server.js` clean; `node -e "require('./server.js')"` loads every dep cleanly (errors only on missing local config — expected per §"Validation environments")
- **`pnpm outdated`**: 1 deprecated direct dep remains — `aws-sdk` v2 (the biggest D-mirror item; 15+ sites; queued last per PLAN ordering). fluent-ffmpeg dropped 2026-05-17 (`7d6bb24fc`); sib-api-v3-sdk dropped 2026-05-17 (`24599a4ac`); moment + moment-timezone dropped 2026-05-17 (`9eec44fa8`); `moment` stays as a transitive of `bunyan@1.8.15` (logger; bunyan→pino is a PLAN-deferred swap, not in current scope).
...
- **What's QUEUED** (in PLAN-recommended order, smallest first):
  ...
  - **NEXT** `aws-sdk` v2 → v3 (15+ sites; biggest scope, last by design). Per-service `@aws-sdk/client-*` imports + new client/command pattern. **Note**: SRS code that lands in W1 is already written against v3 per locked decision #4 / #16 — this task converts the **rest** of cur8-api.
...
- **Resume command from a fresh chat**: `cd ~/Code/cur8-api && git pull && pnpm install` — verify lint + node-check clean, then tackle the last D-mirror item: **`aws-sdk` v2 → v3**. 15+ sites across controllers/, vendor/, tasks/. Per-service imports (`@aws-sdk/client-s3`, `@aws-sdk/client-ses`, `@aws-sdk/client-sns`, etc.) + new client/command pattern. SRS code that lands in W1 is already v3 per locked decision #4 / #16 — this converts the rest. After this lands, Wave D-mirror queue is complete and **W1 SRS** (the real critical path) starts.
