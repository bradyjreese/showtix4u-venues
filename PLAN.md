...
- **HEAD**: `8909e8ba5` `refactor(deps): migrate aws-sdk v2 → v3 per-service clients (Wave D-mirror)` (pushed to `origin/feat/upgrade-2026q2`)
- **Base**: `staging`
- **Build**: lint clean (0 errors, 22 warnings — unchanged through all 7 dep-replacement commits); `node --check server.js` clean; `node -e "require('./server.js')"` loads every dep cleanly (errors only on missing local config — expected per §"Validation environments")
...
- **Commits this program**: 11 on the branch since cutting off `staging` (4 prior + 7 Wave D-mirror dep replacements). **Wave D-mirror queue is COMPLETE.**
...
- **Workstreams not yet started** (Wave D-mirror is now COMPLETE — these are the remaining program workstreams):
  - **W1 SRS port** (the original critical path — now active). Lands on top of the modernized dep surface. ~1.5–2 weeks; RTMP router is the long pole. See §"Workstream 1" for the full task list.
...
- **Resume command from a fresh chat**: `cd ~/Code/cur8-api && git pull && pnpm install` — verify lint + node-check clean, then start **W1 SRS port** (the original critical path; ~1.5–2 weeks). Salvage source: `feature/streaming-service` (port, don't rebase). Key files to port: `controllers/srs.js`, `vendor/srs.js`, `streaming-service/*`, `vendor/ecs.js` (SRS task helpers), `vendor/s3.js` (HLS path support). Build SRS infra in CDK (locked decision #1; no terraform). Add neutral `streaming_streams` table (W1 §7); add `db/streaming-streams.js` mirroring `db/ams-streams.js` exports. Single global `streaming.provider` flag (locked #2). RTMP router as always-on Node-proxy Fargate task (locked #3) — the long pole, ~1 week alone. Tests for everything (the old branch had zero — see W1 §5 list). All SRS code uses `@aws-sdk/*` v3 from day one (locked #4). User provisions infra in `cur8-dev` per their AWS perms (locked #1 / §"Validation environments"). See §"Workstream 1" for the full task list.
