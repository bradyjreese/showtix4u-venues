...
- **HEAD**: `24599a4ac` `refactor(deps): swap sib-api-v3-sdk → @getbrevo/brevo (Wave D-mirror)` (pushed to `origin/feat/upgrade-2026q2`)
- **Base**: `staging`
- **Build**: lint clean (0 errors, 22 warnings — unchanged through 5 dep-replacement commits); `node --check server.js` clean; `node -e "require('./server.js')"` loads every dep cleanly (errors only on missing local config — expected per §"Validation environments")
- **`pnpm outdated`**: 2 deprecated direct deps remain — `aws-sdk` v2, `fluent-ffmpeg`. sib-api-v3-sdk dropped 2026-05-17 (`24599a4ac`); moment + moment-timezone dropped 2026-05-17 (`9eec44fa8`); `moment` stays as a transitive of `bunyan@1.8.15` (logger; bunyan→pino is a PLAN-deferred swap, not in current scope).
...
- **What's QUEUED** (in PLAN-recommended order, smallest first):
  ...
  - **NEXT** `fluent-ffmpeg` (npm-deprecated) → audit usage first; 1 file (`tasks/video-processing.js`). Server-side so the obvious pick is native `child_process` spawn of ffmpeg; `@ffmpeg/ffmpeg` is browser-side.
...
- **Resume command from a fresh chat**: `cd ~/Code/cur8-api && git pull && pnpm install` — verify lint + node-check clean, then continue cur8-api Wave D-mirror replacements at **`fluent-ffmpeg`** (1 file, `tasks/video-processing.js`; audit usage first; server-side so native `child_process` spawn of ffmpeg is the obvious pick over `@ffmpeg/ffmpeg`). Smaller-first ordering applied; aws-sdk v2→v3 stays last.
