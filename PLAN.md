...
- **HEAD**: `4203d479b` `feat(srs): W1 Phase 1b — streaming-service container + infra CDK + docs` (pushed to `origin/feat/upgrade-2026q2`)
- **Base**: `staging`
...
- **Commits this program**: 14 on the branch since cutting off `staging` (4 prior + 7 Wave D-mirror dep replacements + 3 W1 prep/port: Phase 0 `16313884d`, Phase 1a `49a25c725`, Phase 1b `4203d479b`). **Wave D-mirror COMPLETE; W1 Phase 0 + 1 LANDED.**
...
- **Resume command from a fresh chat**: `cd ~/Code/cur8-api && git pull && pnpm install` — verify lint + node-check clean. W1 Phase 0 (`16313884d` neutral table + db) + W1 Phase 1 (`49a25c725` JS port; `4203d479b` container + CDK + docs) already landed. **First: fix 3 Phase 1 findings surfaced by Codex 2026-05-17 audit** (see Execution log entry of the same date): (1) HIGH — HLS naming inconsistency, `streaming-service/entrypoint.sh:99` writes `${STREAM_ID}_adaptive.m3u8` but `vendor/srs.js:256/263/271` + `controllers/srs.js buildVodArchive` use `${streamId}_master.m3u8`; standardize on `_master.m3u8`. (2) MEDIUM — `vendor/srs.js:91`/`:122` createLiveEvent shape: stores `db.streaming_streams.insert()` (which returns the full row, mirroring AMS contract) into a variable named `internal_id`, then returns it as the `internal_id` field; rename to `streamRecord` and extract `.internal_id` properly. (3) MEDIUM — `streaming-service/entrypoint.sh:67` trap only handles SIGTERM/SIGINT; if SRS exits non-zero via `wait $SRS_PID` + `set -e`, the final-sync block at lines 241-250 is skipped (tail HLS files lost). Add `EXIT` to the trap so cleanup runs on any exit. **Then: W1 Phase 2** — extend `vendor/ecs.js` with the SRS task helpers** (`launchSrsStreamTask`, `getSrsTaskPrivateIp`, `stopSrsTask`). This is the first modifies-existing-file SRS work — must preserve the AMS-aware code already in `vendor/ecs.js` (`launchVideoProcessingTask`, `getTask`) and add SRS helpers alongside. Then wire `controllers/srs.js` routes into `routes/api/*` (`/srs/callbacks/{on-publish,on-unpublish,on-hls,on-play}`, `/srs/validate-publish` for the RTMP router contract) — needs `streaming.provider` flag inspection at the route layer so the new routes only fire when the flag is `srs`. After Phase 2: Phase 3 (CDK deploy in cur8-dev — user provisions per locked #1 AWS perms; `cd infra && npm install && cdk synth && cdk deploy`), Phase 4 (RTMP router as a separate always-on Node-proxy Fargate task per locked #3 — the ~1-week long pole), Phase 5 (tests — PLAN §W1 #5 lists 6 categories). All AMS files remain PRESERVED — `terraform/ant-media/`, `controllers/ant-media.js`, `vendor/ant-media.js`, `db/ams-streams.js`, `db/ams-videos.js`, the `ams_streams`/`ams_videos` tables, and the `@antmedia/webrtc_adaptor` on cur8-ui side stay until the post-SRS-prod-validation `chore/ams-removal` follow-up PRs per repo (locked #8 + AMS-infra-hands-off guardrail).
