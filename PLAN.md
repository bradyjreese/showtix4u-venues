...
- **HEAD**: `9dd17811b` `fix(srs): make adaptive-video URL helpers status-aware (require {status})` (pushed to `origin/feat/upgrade-2026q2`)
- **Base**: `staging`
...
- **Commits this program**: 18 on the branch since cutting off `staging` (4 prior + 7 Wave D-mirror + 3 W1 prep/port + 4 audit fix-ups: `ac30504f3` Phase 1, `6cd26c6da` Phase 2-readiness, `18f9e120e` consistency follow-ups, `9dd17811b` status-aware adaptive video URLs). **Wave D-mirror COMPLETE; W1 Phase 0 + 1 LANDED + 4 audit-fix-up commits.**
